...
int main(int argc, char **argv)
{
    if(argc != 3)
    {
        cerr << endl << "Usage: ./sdpl_slam path_to_settings path_to_sequence" << endl;
        return 1;
    }
    ...
    SDPL_SLAM::System SLAM(argv[1],SDPL_SLAM::System::RGBD);
    ...
    //for (int ni = 0; ni < 5; ni++)
    {
        ...
        //imSem.convertTo(imSem, CV_8UC1);

        // Create a 5x5 kernel of ones (you can adjust the size as needed)
       // cv::Mat kernel = cv::Mat::ones(5, 5, CV_32SC1);

        // Dilate the image
       // cv::dilate(imSem, imSem, kernel);

        //imSem.convertTo(imSem, CV_32SC1);
